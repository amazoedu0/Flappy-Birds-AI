...
PIPE_IMG = pygame.transform.scale(PIPE_IMG,(35,300))
...
def draw_windows(win, bird, pipes,base, score,bkg):
    win.blit(BG_IMG,(0,0))
    bkg.draw(win)
    for pipe in pipes:
        pipe.draw(win)
    ...
    text = STAT_FONT.render("Ahmad 002 | Saad 024 | Score: "+str(score),1,(255,255,255))
...
def main():
    ...
    while run:
        ...
        if bird.y + bird.img.get_height() >= 50:
            pass

        bkg.move()
        base.move()
        draw_windows(win, bird,pipes,base,score,bkg)
